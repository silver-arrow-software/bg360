<?php namespace Sas\Forum\Classes;
...
class TopicTracker
{
    ...
    public function getNewTopics($member)
    {
        ...
        $results = Db::table('sas_forum_topics')->select([
                'sas_forum_topics.id',
                'sas_forum_topics.channel_id',
                'sas_forum_topics.last_post_at'
            ])
            ->join('sas_forum_channels', 'sas_forum_channels.id', '=', 'sas_forum_topics.channel_id')
            ->where('sas_forum_topics.last_post_at', '>', $lastLogin)
            ->get()
        ;
